...
cat <<'EOF'
Usage:
  install-pvemcp.sh [--repo PATH] [--client codex|gemini|both] [--no-pip] [--config-dir DIR]
  install-pvemcp.sh [--install-service]
  install-pvemcp.sh --uninstall [--client codex|gemini|both] [--config-dir DIR]
  install-pvemcp.sh --uninstall [--client codex|gemini|both] [--config-dir DIR] --remove-service
...
resolve_service_execution() {
  ...
  if command -v pvemcp-server >/dev/null 2>&1; then
    SERVICE_EXEC_COMMAND="$(command -v pvemcp-server)"
    return 0
  fi

  if [[ -x "${REPO_DIR}/.venv/bin/pvemcp-server" ]]; then
    SERVICE_EXEC_COMMAND="${REPO_DIR}/.venv/bin/pvemcp-server"
    return 0
  fi
...
resolve_mcp_command() {
  if command -v pvemcp-server >/dev/null 2>&1; then
    MCP_COMMAND="$(command -v pvemcp-server)"
  elif [[ -x "${REPO_DIR}/.venv/bin/pvemcp-server" ]]; then
    MCP_COMMAND="${REPO_DIR}/.venv/bin/pvemcp-server"
  else
    MCP_COMMAND="pvemcp-server"
  fi
}
...
target.write_text(
    json.dumps({"mcpServers": {"pvemcp": server}}, indent=2) + "\n",
    encoding="utf-8",
)
PY
}
...
write_service_unit() {
  ...
  cat > "${target_path}" <<EOF
[Unit]
Description=pvemcp MCP server
After=network.target

[Service]
Type=simple
User=root
Environment=PVEMCP_AUDIT_LOG=${AUDIT_LOG}
Environment=PVEMCP_ALLOW_BREAK_GLASS=1
${extra_env}
...
source_server = source_servers.get("pvemcp")
if not isinstance(source_server, dict):
    raise ValueError("Invalid source config: expected mcpServers.pvemcp object")
...
existing_server = mcp_servers.get("pvemcp", {})
...
mcp_servers["pvemcp"] = merged_server
...
remove_systemd_service() {
  ...
  if ${SUDO}systemctl list-unit-files pvemcp.service >/dev/null 2>&1; then
    ${SUDO}systemctl stop pvemcp.service || true
    ${SUDO}systemctl disable pvemcp.service || true
    ${SUDO}rm -f /etc/systemd/system/pvemcp.service
    ${SUDO}systemctl daemon-reload
    echo "Removed pvemcp systemd unit."
  else
    echo "No pvemcp systemd unit found."
  fi
}
...
AUDIT_LOG="${CONFIG_DIR}/logs/pvemcp-audit.log"
...
if "${UNINSTALL}"; then
  if [[ "${CLIENT}" == "codex" || "${CLIENT}" == "both" ]]; then
    if [[ -f "${CONFIG_DIR}/mcp/pvemcp-codex.json" ]]; then
      rm -f "${CONFIG_DIR}/mcp/pvemcp-codex.json"
      echo "Removed ${CONFIG_DIR}/mcp/pvemcp-codex.json"
    else
      echo "No Codex file at ${CONFIG_DIR}/mcp/pvemcp-codex.json"
    fi

    if cleanup_codex_settings "${CONFIG_DIR}/settings.json"; then
      echo "Removed pvemcp from ${CONFIG_DIR}/settings.json"
    else
  ...
  if [[ "${CLIENT}" == "gemini" || "${CLIENT}" == "both" ]]; then
    if [[ -f "${CONFIG_DIR}/mcp/pvemcp-gemini.json" ]]; then
      rm -f "${CONFIG_DIR}/mcp/pvemcp-gemini.json"
      echo "Removed ${CONFIG_DIR}/mcp/pvemcp-gemini.json"
    else
      echo "No Gemini file at ${CONFIG_DIR}/mcp/pvemcp-gemini.json"
    fi
  fi
  ...
  echo "pvemcp uninstall complete."
  exit 0
fi
...
if [[ "${CLIENT}" == "codex" || "${CLIENT}" == "both" ]]; then
  write_client_config "${CONFIG_DIR}/mcp/pvemcp-codex.json" ""
  echo "Wrote Codex config to ${CONFIG_DIR}/mcp/pvemcp-codex.json"
  if sync_codex_settings "${CONFIG_DIR}/settings.json" "${CONFIG_DIR}/mcp/pvemcp-codex.json"; then
...
if [[ "${CLIENT}" == "gemini" || "${CLIENT}" == "both" ]]; then
  write_client_config "${CONFIG_DIR}/mcp/pvemcp-gemini.json" ""
  echo "Wrote Gemini config to ${CONFIG_DIR}/mcp/pvemcp-gemini.json"
fi
...
if [[ "${INSTALL_SERVICE}" == "true" ]]; then
  ...
  if ! resolve_service_execution; then
    echo "Cannot resolve a pvemcp execution target for systemd."
    echo "Install with pip (default) or run from an environment with pvemcp-server on PATH."
    exit 1
  fi
  ...
  ${SUDO}cp "${SERVICE_UNIT}" /etc/systemd/system/pvemcp.service
  ${SUDO}systemctl daemon-reload
  ${SUDO}systemctl enable --now pvemcp.service
  ${SUDO}systemctl status pvemcp.service --no-pager
  echo "Systemd service installed and started."
fi

cat <<EOF
pvemcp install complete.
Config directory: ${CONFIG_DIR}/mcp
Native Codex settings: ${CONFIG_DIR}/settings.json
Executable: pvemcp-server
EOF
